...
function getCustomTagsAndColors(): { tags: string[], sidebarTags: string[], calendarTags: string[], colors: { [tag: string]: string } } {
...
class NotesViewProvider implements vscode.WebviewViewProvider {
    ...
    private async readNotesFromFolder(notesFolder: string, allDeadlines: Set<string>, excludeFolders: string[] = [], calendarTags: string[] = [], defaultFolderName?: string): Promise<any> { 
        ...
        const rootFilesWithStats = await Promise.all(
        ...
        for (const { file } of rootFilesWithStats) {
            ...
            } catch (e) { log(`Could not read file data from ${file}: ${e}`); }
        }
        ...
        for (const folder of folders) {
            ...
            } catch (e) { log(`Could not read folder ${folderPath}: ${e}`); }
